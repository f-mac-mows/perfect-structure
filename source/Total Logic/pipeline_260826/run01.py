...
"""1번 Task 통합 실행기 — 기사 원본 한 개(파일) → claims.jsonl 한 번에.
    ...
    xlsx/csv/json    articles     articles_clean    sentences      A→B(LLM)→C→D→E      (다음 단계 입력)

기존 모듈을 그대로 호출한다 — 이 파일에는 파이프라인 로직이 없다(배선만 한다).

**2026-08-26 통합 갱신(106차)**: 1번 모듈이 flat 파일에서 `src/` 서브패키지로 바뀌었다.
  - import가 전부 `from src import ...` — 루트의 `config.py`(팀장 확장본, client.py가
    사용)와 이름 충돌 없이 공존한다. 루트 `llm_meter.py`는 이제 참조되지 않는 구본이다.
  - Stage B가 **동결 구성**으로 고정됐다: HCX-007 · thinking low · 문단 맥락 ·
    prompt extract_v2.2 (dev 18 F1 0.982 · test 40 F1 0.762 · 새 블라인드 25 F1 0.817).
  - 산출 계약이 v0.5 **17필드**(claims_schema_1번_v2.md 합의안 그대로)가 됐다 —
    comparison_basis/comparison_period/value_num/sent_id/exclusion_code/approx 포함,
    빈 값은 "". adapter.py가 "있으면 우선 사용"으로 이미 배선해 둔 바로 그 포맷이다.
  - 제도 기준값 룰(세율·요율·한도 → 비통계 제외)이 기본 켜짐.
  - 캐시 키에 모델·thinking이 들어간다(cache_tag) — 구성 변형이 옛 응답을 조용히
    재생하는 사고 방지.

단일 URL 서비스 경로(크롤 → … → 검색·판정 → 게시글 JSON)는 `run01_url.py`가 담당한다.

사용:
    # 선별 기사 60건(xlsx) 전 구간
    venv\\Scripts\\python.exe run01.py --input D:/part1/articles.xlsx --outdir data/run

    # 크롤링 기사 1건 (파일로 받은 경우)
    venv\\Scripts\\python.exe run01.py --input crawled.json --outdir data/one

    # LLM 호출 없이 배선만 확인 (P2까지)
    venv\\Scripts\\python.exe run01.py --input D:/part1/articles.xlsx --outdir data/run --to p2

    # 중단 후 재개 — 캐시가 성공분을 재생하므로 재과금 0
    venv\\Scripts\\python.exe run01.py --outdir data/run --from p3
"""
...
from src import config, llm_meter, p0_load, p1_clean, p2_split
from src.p3_emit import AccountingError
from src.p3_pipeline import CIRCUIT_BREAKER_RATE

STAGES = ("p0", "p1", "p2", "p3")

# 실측 단가 — HCX-007(thinking low) dev 18기사 기준(92차: 회당 4,350원/약 278문장).
# 기사당·문장당 둘 다 두고 **큰 쪽**을 추정치로 쓴다: 나열형 기사는 기사당 숫자 문장이
# 평균의 2배라 기사 기준만 쓰면 실제의 절반으로 과소 추정된다.
# 어느 쪽도 정확하지 않은 추정이며, 캐시 재생분은 과금 0이라 그만큼 과대 추정된다.
COST_PER_ARTICLE_KRW = 245.0
COST_PER_SENTENCE_KRW = 16.0
DEFAULT_BUDGET_KRW = 5000.0    # 추정 요금이 이 값을 넘으면 --yes 없이는 실행하지 않는다

# 동결 구성(106차) — 튜닝이 끝난 조합. 바꾸면 측정된 성적과 다른 물건이 된다.
FROZEN_MODEL = "HCX-007"
FROZEN_THINKING = "low"
...
def build_extractor(outdir: Path, cache_path: Path, *, stub: bool, fresh: bool, meter):
    ...
    stub=True: 골든 라벨을 되돌려주는 무-LLM 추출기(배선 검증용). 골든셋 파일이 필요하다.
    """
    if stub:
        from src.p3_pipeline import make_golden_stub_extractor
        return make_golden_stub_extractor()

    import src.p3_stage_b as sb
    from src.p3_cache import ReplayCache

    client = sb.HCXClient(meter=meter, model=FROZEN_MODEL, thinking=FROZEN_THINKING)
    # 키에 cache_tag(모델+thinking)를 쓴다 — 구성이 바뀌면 캐시가 갈라져야
    # 옛 응답이 조용히 재생되지 않는다(97차 교훈).
    cache = ReplayCache(cache_path, sb.PROMPT_VERSION, client.cache_tag)
    sent_index = sb.build_sentence_index(outdir / "sentences.jsonl")
    system_prompt = sb.load_system_prompt()     # 개발 메타 스트립본(60차 — 매 호출 과금 절감)
    print(f"Stage B: {client.cache_tag} · {sb.PROMPT_VERSION} · 캐시 {len(cache)}건"
...
def stage_p3(outdir: Path, cache_path: Path, *, stub: bool, fresh: bool, meter,
    ...
    제도 기준값 룰(statutory_rule)은 p3_pipeline.run의 기본값 True를 그대로 쓴다(106차 채택).
    """
...
def _count_records(path: Path, article_filter: set | None = None,
    ...
    from src.p3_stage_a import is_numeric_sentence
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--cache", type=Path, default=None,
                    help="record-replay 캐시 경로 (기본: cache/replay_extract_v1.jsonl)")
    ...
    ap.add_argument("--meter", type=Path, default=None,
                    help="사용량 로그 경로 (기본: data/llm_usage.jsonl)")
    ...
    print(f".env: {env_path if env_path.is_file() else '없음 — 키가 필요한 단계에서 멈춘다'}")
    try:
    ...
    if summary is not None:
        ...
        print(f"\n{args.end.upper()}까지 완료 — 이어서 돌리려면 "
              f"`run01.py --outdir {args.outdir} --from {STAGES[STAGES.index(args.end) + 1]}`")
    return 0


def _handoff_claims(summary: dict) -> None:
    """P3까지 끝나 claims.jsonl이 나오면, 다음 단계(run04_local.py)가 --input 없이 바로
